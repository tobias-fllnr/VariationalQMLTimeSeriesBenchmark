...
def train_and_analyse(version, model_name, num_qubits, seq_length, ansatz, data_label, random_id, learning_rate, hidden_size, prediction_step, batch_size):
    ...
    (
        ...
        inputs_testing,
        labels_testing,
    ) = data_handler.get_training_and_test_data()
    ...
    analyzer.plot_cost(
        cost_training=cost_training, cost_validation=cost_validation, cost_testing=cost_testing
    )
    analyzer.evaluate_trained_model(
        inputs_testing, labels_testing, inputs_validation, labels_validation
    )
    analyzer.save_training_info(
        cost_training=cost_training,
        cost_validation=cost_validation,
        cost_testing=cost_testing,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="version of the code, in case of change of models, training or data",
    )
    parser.add_argument(
        "-model",
        "--model_name",
        type=none_or_type(str),
        help="model to train e.g. vqc, qlstm_paper, qlstm_linear_enhanced_paper, qrnn_paper, lstm, rnn, mlp",
    )
    parser.add_argument(
        "-data",
        "--data_label",
        type=none_or_type(str),
        help="data e.g. lorenz_1000, mackey_glass_1000_default, henon_map_1000_default",
    )
    parser.add_argument(
        "-id", "--random_id", type=none_or_type(int), help="random id to initialize the weights"
    )
    parser.add_argument(
        "-lr", "--learning_rate", type=none_or_type(float), help="step size of the Adam optimizer"
    )
    parser.add_argument(
        "-num_qubits", "--number_qubits", type=none_or_type(int), help="number of qubits used"
    )
    parser.add_argument(
        "-hidden_size",
        "--hidden_size",
        type=none_or_type(int),
        help="hidden size of the rnn, lstm, qlstm_linear_enhanced_paper, qrnn_paper",
    )
    parser.add_argument(
        "-ansatz", "--type_of_ansatz", type=none_or_type(str), help="type of quantum circuit ansatz"
    )
    parser.add_argument(
        "-seq_length", "--sequence_length", type=none_or_type(int), help="sequence length of data"
    )
    parser.add_argument(
        "-pred_step",
        "--prediction_step",
        type=none_or_type(int),
        help="step into the future on which the models are trained on",
    )
    parser.add_argument(
        "-batch_size", "--batch_size", type=none_or_type(int), help="batch size for training"
    )
