                    ...
                    if string.startswith("E"):
                        ...
                        if input_col is not None and rotation_gate is not None:
                            for j in range(self.num_qubits):
                                angle = (
                                    torch.pi * inputs[:, input_col] * (3 ** (j - self.num_qubits))
                                )
                        ...
                        qml.CRX(
                            weights[i][self.num_qubits - 1][param_num_count],
                            wires=[self.num_qubits - 1, 0],
                        )
                        param_num_count += 1
                    elif string == "CY":
                        for j in range(self.num_qubits - 1):
...
class LSTM(nn.Module):
    def __init__(self, hidden_size, ansatz, data_label, random_id=42):
        ...
        if self.data_label.startswith("lorenz"):
            self.lstm = nn.LSTM(
                3, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True
            )
            self.fc = nn.Linear(self.hidden_size, 3)
        elif self.data_label.startswith("henon"):
            self.lstm = nn.LSTM(
                2, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True
            )
            self.fc = nn.Linear(self.hidden_size, 2)
        else:
            self.lstm = nn.LSTM(
                1, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True
            )
